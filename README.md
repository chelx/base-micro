...
![Code Organization](code_org_architecture.png)
...
Để tài liệu API được sinh ra đầy đủ và dễ hiểu, tất cả endpoint cần được gắn các Tag của Swagger chi tiết. Tham khảo trực tiếp từ `AuthController`:
```typescript
...
Các class Entity cần được định nghĩa decorators cẩn thận và quản lý index chuẩn xác, file đặt đuôi `.entity.ts`.
```typescript
@Entity('products')
